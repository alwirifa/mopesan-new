...
const Page = ({
  ...
  const handleSave = async () => {
    try {
      ...
      let url = `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/auth/orders/tax-report?sort=${sort}&offset=${offset}&limit=${limit}`;
      if (startDate) {
    ...
    <div className="">
      ...
      <div className="h-full w-full mt-8 p-8 bg-white rounded-lg flex flex-col gap-4">
        ...
        <div className="flex gap-4">
          ...
          <div className="p-4 rounded-md border flex-1 flex flex-col gap-2">
            ...
            <p className="text-xl font-semibold text-green-900">
              {taxData?.total_sales &&
                formatCurrency(taxData?.total_sales)}
            </p>
          </div>
          <div className="p-4 rounded-md border flex-1 flex flex-col gap-2">
            <p className="text-sm text-textGray">Total Service Fee</p>
            <p className="text-xl font-semibold  text-green-900">
              {taxData?.total_service_fee && formatCurrency(taxData.total_service_fee)}
            </p>
          </div>
          <div className="p-4 rounded-md border flex-1 flex flex-col gap-2">
        ...
        <div className="w-full flex justify-end mt-4">
          <Pagination
            totalPages={Math.ceil(taxData.total_transaction / limit)}
          />
        </div>
      </div>
    </div>

...
const Card = ({ id }: { id: number }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      let url = `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/auth/orders/merchant/${id}/between?sort=${sort}&page=${currentPage}&limit=${limit}`;
      if (startDate) {
  ...
  const handleDownload = () => {
    window.open(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/export?type=merchant-orders&sort=${sort}&start_date=${startDate}&end_date=${endDate}&merchant_id=${id}`
    );
  };
